package by.mybooks.ui.homepage;

public class HomePageXpath {

    public static final String BUTTON_ACCEPT_ALL_COOKIES_XPATH = "//button[@data-button-ck-action='accept-all']";

    public static final String PAYMENT_AND_DELIVERY_LINK_XPATH = "//span[text()='Оплата и доставка']";
    public static final String PAYMENT_AND_DELIVERY_TEXT_XPATH = "//h1[text()='Оплата и доставка']";

    public static final String ADDRESS_OF_THE_SHOP_LINK_XPATH = "//span[text()='Адрес магазина']";
    public static final String ADDRESS_OF_THE_SHOP_TEXT_XPATH = "//div[@style=' padding: 20px; font-family: -webkit-body; ']" +
            "//span[contains(text(), 'Г. МИНСК, ОКТЯБРЬСКАЯ ПЛ. 2, ТЦ КУПАЛОВСКИЙ')]";

    public static final String SEARCH_STRING_XPATH = "//input[@placeholder='поиск']";
    public static final String BUTTON_SEARCH_XPATH = "//button[@type='submit']";
    public static final String BUTTON_BASKET_XPATH = "//div[@class='ok-shcart-box']";
    public static final String BUTTON_NEXT_XPATH = "//i[@class='fa fa-chevron-right']";

    public static final String TITLE_TEXT_XPATH = "//h1[@class='title']";

    public static final String IN_STOCK_LINK_XPATH = "//a[@class='ok-menu-top__href']//span[text()='В наличии']";
    public static final String NEW_YEAR_LINK_XPATH = "//a[@class='ok-menu-top__href']//span[text()='Новый год']";
    public static final String NEW_PRODUCTS_LINK_XPATH = "//a[@class='ok-menu-top__href']//span[text()='Новинки']";
    public static final String BOOKS_FOR_CHILDREN_LINK_XPATH = "//a[@class='ok-menu-top__href']//span[text()='Книги для детей']";
    public static final String FICTION_LINK_XPATH = "//a[@class='ok-menu-top__href']//span[text()='Художественная литература']";
    public static final String ESOTERICISM_LINK_XPATH = "//a[@class='ok-menu-top__href']//span[text()='Эзотерика']";
    public static final String PSYCHOLOGY_LINK_XPATH = "//a[@class='ok-menu-top__href']//span[text()='Психология']";
    public static final String PHILOSOPHY_SOCIOLOGY_LINK_XPATH = "//span[text()='Философия. Социология']";
    public static final String BESTSELLERS_LINK_XPATH = "//span[text()='Бестселлеры']";
    public static final String MARKDOWN_LINK_XPATH = "//span[text()='Уценка']";
    public static final String HISTORY_LINK_XPATH = "//span[text()='История']";
    public static final String URSS_SCIENCE_EDUCATION_JURISPRUDENCE_LINK_XPATH = "//span[text()='УРСС Наука, образование, юриспруденция']";
    public static final String BUSINESS_LITERATURE_LINK_XPATH = "//span[text()='Деловая литература']";
    public static final String FOR_CHILDREN_AND_PARENTS_LINK_XPATH = "//span[text()='Детям и родителям']";
    public static final String EDUCATIONAL_LITERATURE_LINK_XPATH = "//span[text()='Учебная литература']";
    public static final String ART_CULTURE_LINK_XPATH = "//span[text()='Искусство. Культура']";
    public static final String COMPUTER_LITERATURE_LINK_XPATH = "//span[text()='Компьютерная литература']";
    public static final String MEDICAL_LITERATURE_LINK_XPATH = "//span[text()='Медицинская литература']";
    public static final String NONFICTION_LINK_XPATH = "//span[text()='Нехудожественная литература']";
    public static final String HUMANITIES_PUBLISHING_HOUSES_LINK_XPATH = "//span[text()='Гуманитарные издательства']";
    public static final String OTHER_PRODUCTS_LINK_XPATH = "//span[text()='Прочие товары']";
    public static final String HOME_CIRCLE_LINK_XPATH = "//span[text()='Домашний круг']";
    public static final String MANN_IVANOV_FERBER_LINK_XPATH = "//span[text()='МАНН, ИВАНОВ И ФЕРБЕР']";
}
